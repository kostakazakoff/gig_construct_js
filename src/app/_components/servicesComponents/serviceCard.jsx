...
export default function ServiceCard({ service, language }) {
    ...
    return (
        <Link key={service.id} href={`/services/${service.id}`} id={service.category} passHref>
            <article className="group grid grid-rows-4 max-w-96 h-80 transition duration-300 ease-in-outhover:translate-y-1 hover:scale-105 shadow-md/30 hover:shadow-xl/40 rounded-md overflow-hidden text-slate-200 border border-gig-blue dark:border-slate-300" lang={language.toLowerCase()}>
