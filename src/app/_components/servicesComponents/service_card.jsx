...
export default function ServiceCard({ service, language }) {
    return (
        <Link key={service.id} href={`/services/${service.id}`} passHref>
            <article className="relative group w-96 h-96 transition duration-300 ease-in-outhover:translate-y-1 hover:scale-105 shadow-md hover:shadow-xl rounded-md overflow-hidden dark:text-slate-200 hover:border-1 border-slate-800 dark:hover:border-0" lang={language.toLowerCase()}>
                <div className="group-hover:overflow-hidden align-middle text-slate-200 group-hover:text-slate-800 dark:group-hover:text-slate-200">
                    <div>
                        <div className="absolute inset-0 bg-gig-blue/80 group-hover:bg-slate-300/80 dark:group-hover:bg-slate-800/80" />
                    </div>
                    <Image
                        alt=""
                ...
                <div className="absolute w-96 h-30 grid grid-rows-2 grid-cols-3 bottom-0 text-base/6 opacity-0 transition-opacity duration-400 group-hover:opacity-100 group-hover:border-t-1 border-slate-400 group-hover:shadow-lg/30">
