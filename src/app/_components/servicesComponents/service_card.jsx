...
export default function ServiceCard({ service, language }) {
    return (
        <Link key={service.id} href={`/services/${service.id}`} passHref>
            <article className="group grid grid-rows-3 w-96 h-80 transition duration-300 ease-in-outhover:translate-y-1 hover:scale-105 shadow-md hover:shadow-xl/40 rounded-md overflow-hidden text-slate-200" lang={language.toLowerCase()}>
